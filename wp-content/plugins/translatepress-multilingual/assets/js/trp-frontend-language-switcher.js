...
class ShortcodeSwitcher extends BaseSwitcher {
  constructor(wrapper) {
    const overlay =
      ...
      [...wrapper.querySelectorAll(".trp-language-switcher")].find((el) =>
        el.classList.contains("trp-shortcode-overlay")
      );
...
class FloaterSwitcher extends BaseSwitcher {
  constructor(el) {
    ...
    this.onDocClick = (evt) => {
      if (!el.contains(evt.target)) this.setOpen(false, { source: evt });
    };
    document.addEventListener("click", this.onDocClick, true);
  }
}
...
function initLanguageSwitchers(root = document) {
  root
    ...
    .forEach((el) => {
      if (!isMarked(el)) {
        mark(el);
        new FloaterSwitcher(el);
      }
    });

  root
    .querySelectorAll(".trp-shortcode-switcher__wrapper")
    .forEach((wrapper) => {
      const overlay = wrapper.querySelector(
        ".trp-language-switcher:not(.trp-opposite-button)"
      );
